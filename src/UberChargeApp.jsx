...
import { MapPin, Zap, Clock, Star, Battery, Navigation, X } from 'lucide-react';
...
const UberChargeApp = () => {
  ...
  // ---- Reward points (1 € = 10 points), computed from estimatedCost
  const POINTS_PER_EURO = 10;
  const parseEuro = (s) => {
    if (!s) return 0;
    const cleaned = String(s).replace(/[^\d.,-]/g, '').replace(',', '.'); // "€8,50" -> "8.50"
    const val = parseFloat(cleaned);
    return isNaN(val) ? 0 : val;
  };
  const costToPoints = (costStr) => Math.round(parseEuro(costStr) * POINTS_PER_EURO);

  // Mock charging station data (bonus removed; we compute points from estimatedCost)
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-green-50 to-gray-50">
            ...
            {showRecommendation && (
                <svg className="absolute inset-0 w-full h-full pointer-events-none">
                  <path d="M 50% 50% Q 55% 40% 60% 45%" stroke="#000" strokeWidth="3" fill="none" strokeDasharray="8,4" />
                </svg>
            )}
          </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-4">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center text-sm">
                      ...
                      <span className="text-gray-600">{recommendedStation.rating}</span>
                      <span className="mx-2 text-gray-400">•</span>
                      <span className="text-green-600 font-medium">+{recommendedPoints} reward points</span>
                    </div>
                  </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-green-600 text-sm">
                            Calculated from estimated cost (1€ = 10 pts)
                          </div>
                        </div>
